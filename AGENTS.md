# Scissors ASP Agent Guide

## Scope

These instructions apply to the entire repository. Scissors ASP is a patch-based fork of
[AdvancedSlimePaper](https://github.com/InfernalSuite/AdvancedSlimePaper) (ASP) built with paperweight. Paperweight
applies Paper, then ASP, then Scissors. The files produced by applying patches are development worktrees, not the
...
- The `scissors-asp` root project and the `scissors-api` and `scissors-server`
  modules.
...
AdvancedSlimePaper is the direct upstream and Paper remains ASP's upstream. Keep `AdvancedSlimePaper`, `ASPaper`,
`aspaper-api`, `aspaper-server`, `Paper`, `paper-api`, `paper-server`, `com.infernalsuite.asp`, `io.papermc`, upstream
repositories, APIs, and Minecraft names where they describe upstream code or compatibility. Do not perform blanket
upstream-to-Scissors replacements. Preserve upstream copyright, license, and attribution text.

ASP's slime `api` and `core` projects are upstream-owned modules that the aspaper build depends on. This repository
materializes them as the `:api` and `:core` projects: their sources come from the generated `asp-api/` and `asp-core/`
worktrees, and their build scripts are generated from the tracked `api/build.gradle.kts.patch` and
`core/build.gradle.kts.patch`. Those patches replace ASP's buildSrc convention plugins with standalone build scripts;
the dependency versions inlined there mirror ASP's `gradle/libs.versions.toml` and must be revalidated on every
upstream update.

The ASP commit is pinned by `aspRef` in `gradle.properties`. ASP's branch naming is misleading: `paper_upstream` is the
branch that carries the current ASP-on-Paper patches, so `aspRef` must be a commit on that branch. The ASP commit
determines the Paper commit beneath it (`paperRef` in ASP's `gradle.properties`), and `paperApiVersion` in this
repository's `gradle.properties` must name the published Paper API build that matches that Paper commit (copy the
`version` property from ASP's `gradle.properties`). Do not change these relationships unless the task is specifically
an upstream update.
...
- [AdvancedSlimePaper](https://github.com/InfernalSuite/AdvancedSlimePaper) is Scissors ASP's direct upstream. The
  `paper_upstream` branch carries the ASP patches this fork builds on. Resolve update behavior against the exact
  `aspRef`, preferably through paperweight's local ASP checkout.
- [Paper](https://github.com/PaperMC/Paper) is ASP's upstream. Use the exact `paperRef` pinned by the selected ASP
  commit when investigating Paper-owned code; do not substitute Paper's current branch.
...
- [paperweight-examples](https://github.com/PaperMC/paperweight-examples) provides official fork examples, but it is
  frequently behind current Paper, ASP, and paperweight behavior. Treat it as a lead and revalidate every task name
...
## Updating AdvancedSlimePaper

Treat an ASP update as a patch rebase, not a dependency bump.
...
2. Set `aspRef` in `gradle.properties` to the full target ASP commit SHA. The commit must be on the `paper_upstream`
   branch.
3. Copy the `version` property from the target ASP `gradle.properties` into `paperApiVersion`. If the target commit
   changes Minecraft versions, also copy `mcVersion` and `apiVersion` from the target ASP `gradle.properties`. Do not
   guess these values.
4. Compare the target ASP root build files, `gradle/libs.versions.toml`, buildSrc conventions, Gradle wrapper,
   Java/toolchain requirements, and paperweight configuration with this repository. Port only changes required to
   build the new target; retain Scissors project names, group, branding, repositories, and owned build behavior. In
   particular, revalidate the dependency versions inlined in `api/build.gradle.kts.patch` and
   `core/build.gradle.kts.patch` against the target's version catalog.
5. Run `./gradlew applyAllPatches`. Resolve failures in patch-set order:
    - Rebase rejected single-file build-script changes in the generated `scissors-*/build.gradle.kts`,
      `api/build.gradle.kts`, and `core/build.gradle.kts`, then run `./gradlew rebuildAspaperSingleFilePatches`.
    ...
    - Preserve the reason for each Scissors patch. Drop a patch when ASP or Paper now provides the same behavior; do
      not retain an empty or redundant compatibility patch.
    - Use `applyAspaperSingleFilePatchesFuzzy` or `applyPaperApiFilePatchesFuzzy` only to recover changed context that is
...
6. Regenerate the tracked patch sets with the full patch-regeneration sequence below. Review every regenerated patch;
   an upstream update must not silently absorb unrelated ASP or Paper changes into a Scissors patch.
...
`aspRef`, any required version or build-tooling changes, and all regenerated Scissors patches needed for that exact
ASP commit.
...
| `api/build.gradle.kts.patch`     | Build patch for ASP's slime `api` project                                                              | Yes        |
| `core/build.gradle.kts.patch`    | Build patch for ASP's slime `core` project                                                             | Yes        |
...
| `asp-api/`, `asp-core/`          | Applied slime module worktrees generated by paperweight                                                | No         |
| `asp-server/`                    | Applied worktree with ASP's directly owned server sources                                              | No         |
...
| `api/build.gradle.kts`           | Generated patched build script                                                                         | No         |
| `core/build.gradle.kts`          | Generated patched build script                                                                         | No         |
...
4. Choose the correct layer before editing:
    ...
    - ASP slime module source: `asp-api/` or `asp-core/`
    - ASP directly owned server source: `asp-server/`
...
|--------------|--------------------------------|-----------------------------|
| API          | `fixupPaperApiFilePatches`     | `rebuildAspaperPatches`         |
...
- Do not update ASP, Paper, Gradle, paperweight, Java, mappings, or dependencies unless requested.
